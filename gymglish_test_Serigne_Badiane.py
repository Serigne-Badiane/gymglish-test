...
def count_occurrences_in_text(word, text):
    ...
    text = text.replace(".", ' cut')
                        # Convert word and string in small caps
    word = word.lower()
    word_split = word.split()
    nb_words = len(word_split)
    text = text.lower()
    ...
    if nb_words == 1 :
        ...
        for i in range(len_text - nb_words + 1):
            current_sequence = ' '.join(words_in_text[i:i + nb_words])
...
def doit():
    ...
    """
    text = """in the break room. 'Milk and sugar'"""
    assert 0 == count_occurrences_in_text("maley", "John O'maley is my friend")
    assert 0 == count_occurrences_in_text("sugar",text)
